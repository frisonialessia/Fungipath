// Ilustraciones SVG generativas por especie.
// Portado de spIllust() del prototipo: genera un SVG reconocible según
// 'shape' + los colores de cada especie. Estética editorial, no infantil.
...
export function spIllust(sp: Species, size = 88): string {
  ...
  if (sh === "bolete") {
    ...
    g += `<ellipse cx="32" cy="32" rx="12" ry="7" fill="#fff" opacity=".15"/>`;
  } else if (sh === "amanita") {
    ...
    if (sp.dots !== false) {
      const pts = [[24,30],[34,22],[44,18],[54,22],[64,30],[30,42],[44,38],[58,42],[38,50],[50,50]];
      pts.forEach(p => { g += `<circle cx="${p[0]}" cy="${p[1]}" r="2.4" fill="#f1e7db"/>`; });
    }
  } else if (sh === "chanterelle") {
    g += `<path d="M16 30 Q44 16 72 30 Q66 44 58 90 Q52 102 44 102 Q36 102 30 90 Q22 44 16 30 Z" fill="${cap}"/>`;
    g += `<path d="M22 40 Q44 50 66 40 M30 60 Q44 66 58 60 M34 78 Q44 82 54 78" fill="none" stroke="${cap2}" stroke-width="1.3" opacity=".45"/>`;
  } else if (sh === "morel") {
    ...
    let al = "";
    for (let y = 18; y < 62; y += 9) { for (let x = 32; x < 58; x += 8) { al += `<ellipse cx="${x}" cy="${y}" rx="3.2" ry="4.2" fill="none" stroke="${cap2}" stroke-width="1.2"/>`; } }
    g += al;
  } else if (sh === "oyster") {
    g += `<path d="M14 52 Q14 26 50 26 Q84 28 78 48 Q70 66 44 66 Q22 66 14 52 Z" fill="${cap}"/>`;
    let ln = "";
    for (let a = 22; a < 78; a += 6) { ln += `<line x1="46" y1="62" x2="${a}" y2="32" stroke="${cap2}" stroke-width="1" opacity=".4"/>`; }
    g += ln;
  } else if (sh === "parasol") {
    ...
    const sc = [[26,28],[36,24],[50,24],[60,28],[32,38],[44,34],[56,38],[38,46],[50,46]];
    sc.forEach(p => { g += `<circle cx="${p[0]}" cy="${p[1]}" r="2" fill="${cap2}"/>`; });
    g += `<circle cx="44" cy="20" r="4" fill="${cap2}"/>`;
  } else if (sh === "puffball") {
    g += `<ellipse cx="44" cy="64" rx="34" ry="32" fill="${cap}"/><ellipse cx="34" cy="52" rx="10" ry="8" fill="#fff" opacity=".12"/>`;
  } else if (sh === "trumpet") {
    ...
    const br = [[44,70,30,30],[44,70,58,28],[44,72,24,46],[44,72,64,44],[44,74,38,20],[44,74,52,18]];
    br.forEach(b => { g += `<path d="M${b[0]} ${b[1]} Q${(b[0]+b[2])/2} ${b[1]-26} ${b[2]} ${b[3]}" fill="none" stroke="${cap}" stroke-width="6" stroke-linecap="round"/>`; });
  }
